...
// === Helpers iguales a los de eliminatorias ===
function toDateInput(v) {
  if (!v) return "";
  if (typeof v === "string") return v.slice(0, 10); // "YYYY-MM-DD" o "YYYY-MM-DDTHH:MM..."
  try {
    const d = new Date(v);
...
function toTimeInput(v) {
  ...
  const s = String(v);
  const m = s.match(/(\d{1,2}):(\d{2})/); // agarra primer HH:MM en lo que venga
  if (m) {
...
export default function PartidoModal({ partido, onClose, onSaved }) {
  ...
  useEffect(() => {
  // catálogo de canchas activas (una vez)
  (async () => {
    try {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [r1, r2, rE] = await Promise.all([
          api.get(`/jugadores/${partido.equipo_local_id}`),
          api.get(`/jugadores/${partido.equipo_visita_id}`),
          api.get(`/eventos/${partido.id}`),
        ]);
        const JL = r1.data;
        const JV = r2.data;
        setJugL(JL);
        ...
        // normaliza fecha/hora/cancha desde partido
        setFecha(toDateInput(partido.fecha));
        setHora(toTimeInput(partido.hora));
  ...
  const guardar = async () => {
    try {
      ...
      await api.post("/partidos/editar-datos", {
        partido_id: partido.id,
        fecha: fecha || null,
        hora: toSqlTime(hora), // ← normalizado "HH:MM:00" o null
        cancha: cancha || null,
      });

      // 2) reescribir eventos
      await api.delete(`/eventos/por-partido/${partido.id}`);
      const eventos = [
        ...gL.map((e) => ({ ...e, partido_id: partido.id })),
        ...gV.map((e) => ({ ...e, partido_id: partido.id })),
        ...ams.map((e) => ({ ...e, partido_id: partido.id })),
        ...ros.map((e) => ({ ...e, partido_id: partido.id })),
      ];
      for (const ev of eventos) {
        await api.post("/eventos", { ...ev, minuto: 0 });
      }

      // 3) actualizar marcador
      await api.post("/partidos/actualizar-goles", {
        partido_id: partido.id,
        goles_local: gL.length,
        goles_visita: gV.length,
      });
...
>
  ...
  {/* si hay un valor previo que no está en catálogo, mantenlo */}
  {cancha && !canchas.some(c => c.nombre === cancha) && (
    <option value={cancha}>{cancha} (no listada)</option>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <h4 className="font-semibold mb-1">
              Goles {partido.equipo_local} ({gL.length})
            </h4>
            ...
            <ul className="list-disc ml-4 text-sm">
              {gL.map((e, i) => (
                <li key={i}>{nombre(e.jugador_id)}</li>
              ))}
            </ul>
          </div>
          <div>
            <h4 className="font-semibold mb-1">
              Goles {partido.equipo_visita} ({gV.length})
            </h4>
            <div className="flex items-center gap-2 mb-2">
              <button
      ...
      {modalJugador && (
        <MiniModalJugador
          titulo={`Selecciona jugador para ${modalJugador.tipo} ${
            modalJugador.equipo ? `(${modalJugador.equipo})` : ""
          }`}
          jugadores={
            modalJugador.equipo === "local"
              ? jugL
              : modalJugador.equipo === "visita"
              ? jugV
              : [...jugL, ...jugV]
          }
